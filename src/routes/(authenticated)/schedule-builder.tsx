...
import { CalendarPlus, Check, Cloud, Loader2, RefreshCw, Sparkles } from "lucide-react";
...
function ScheduleBuilderPage() {
  return (
    <Suspense fallback={<ScheduleBuilderSkeleton />}>
      <SeasonSelector />
    </Suspense>
  );
}
...
type ScheduleBuilderContentProps = {
  seasonId: string;
...
function ScheduleBuilderContent({
  ...
  // Mutations
  const generateMutation = useMutation(trpc.matchup.generate.mutationOptions());
  ...
  // Generate matchups handler
  const handleGenerateMatchups = async () => {
    try {
      await generateMutation.mutateAsync({ seasonId });
      await refetch();
    ...
    return (
      <div className="flex h-[calc(100vh-4rem)] items-center justify-center">
        <div className="text-center">
          <Sparkles className="text-muted-foreground/30 mx-auto mb-4 size-16" />
          <h2 className="text-xl font-semibold">No matchups for this season</h2>
          <p className="text-muted-foreground mt-2 mb-6">
            Generate round-robin matchups to start building your schedule
          </p>
