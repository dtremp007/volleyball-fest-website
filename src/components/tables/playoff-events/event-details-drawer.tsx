...
export function PlayoffEventDetailsDrawer() {
  const { playoffEventId } = routeApi.useSearch();
  ...
  return (
    <Drawer open={!!playoffEventId} onOpenChange={handleOpenChange} direction="right">
      <DrawerContent className="h-full overflow-hidden data-[vaul-drawer-direction=right]:sm:max-w-3xl data-[vaul-drawer-direction=right]:xl:max-w-5xl">
        <DrawerHeader className="border-b">
          <div className="flex items-center justify-between">
            <div>
              <DrawerTitle>{event ? event.name : "Playoff event details"}</DrawerTitle>
              <DrawerDescription>
                {event
                  ? formatDate(event.date)
                  : "View scheduled playoff games for this event"}
              </DrawerDescription>
            </div>
            <DrawerClose asChild>
                              ...
                              {slot.courtA ? (
                                <div>
                                  ...
                                  <div className="text-muted-foreground text-xs">
                                    {slot.courtA.label} · {slot.courtA.category}
                                  </div>
                                </div>
                              ) : (
                            ...
                            <TableCell className="whitespace-normal">
                              {slot.courtB ? (
                                <div>
                                  ...
                                  <div className="text-muted-foreground text-xs">
                                    {slot.courtB.label} · {slot.courtB.category}
                                  </div>
                                </div>
                              ) : (
                                <span className="text-muted-foreground text-xs">-</span>
