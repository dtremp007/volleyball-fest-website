...
function patchMatchupSetScore(
  ...
  const sets = existingSet
    ? matchup.sets.map((setScore) => (setScore.set === params.set ? nextSet : setScore))
...
function updateMatchupInList(
  ...
  return matchups.map((matchup) =>
    matchup.id === params.matchupId ? patchMatchupSetScore(matchup, params) : matchup,
  );
}
  ...
  const { mutateAsync: saveSetScore } = useMutation(
    trpc.matchup.saveSetScore.mutationOptions({
      onSuccess: (_result, variables) => {
        ...
        queryClient.setQueryData<MatchupSeasonData>(seasonQueryKey, (currentData) => {
          if (!currentData) return currentData;

          return {
            ...currentData,
            matchups: updateMatchupInList(currentData.matchups, patchParams),
            scheduled: updateMatchupInList(currentData.scheduled, patchParams),
            unscheduled: updateMatchupInList(currentData.unscheduled, patchParams),
          };
        });
      },
      onError: (error) => {
  ...
  const matchupRowsWithSubRows = React.useMemo(() => {
    ...
    return data.matchups
      .map((matchup) => {
        ...
        const subRows: SetSubRow[] = Array.from({ length: matchup.bestOf }, (_, idx) => {
          const set = idx + 1;
